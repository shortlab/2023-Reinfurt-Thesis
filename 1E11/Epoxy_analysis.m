...
for rate=13                                                     % Choose the heating rate to import here
    ...
    %data=readmatrix(strcat(chip_number,'_',num2str(experiment_number(rate)),'_Blank_',max_temperature,'_',num2str(heating_rate(rate)),'Kps.txt'));
     data=readmatrix(strcat('1E11_',num2str(sampnum),'.txt'));
...
for ii=1:segment_number/2-1
    ...
    [mdl,gof] = fit(x,y,ft,'start',[-.5 ,.5, 90,20]);
    Tg(T_index)=mdl.xmid;
    CI=confint(mdl);
...
for ii=2:length(Tg_mult)
    stan_dev_mult(ii)=(std(Tg_mult(1:ii))/sqrt(ii));%./sqrt(ii);
end
...
%%
writematrix(Tg_mult, 'Tg_1E11.dat')
writematrix(Tg_mult_std, 'Tg_std_1E11.dat')
