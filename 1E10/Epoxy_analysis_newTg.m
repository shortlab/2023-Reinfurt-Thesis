...
for rate=13                                                     % Choose the heating rate to import here
    ...
    %data=readmatrix(strcat(chip_number,'_',num2str(experiment_number(rate)),'_Blank_',max_temperature,'_',num2str(heating_rate(rate)),'Kps.txt'));
     data=readmatrix(strcat('1E10_',num2str(sampnum),'.txt'));
...
T_index=1;
testsse=zeros(1,49);
model_err=zeros(1,49);

% fit sigmoid function
tempiso=temp_sample_heat(701:2501,2);

for ii=1:segment_number/2-1

    if mod(ii,2)==0
    heatiso=heatflow_heat(701:2501,ii);
...
sample_w(sampnum)=1/(stan_dev(end).^2);
...
%for ii=1:length(Tg_mult)
    ...
    %if abs(Tg_mult(ii-count)-avg_Tg_mult)>=5*stan_dev_mult(end)
        %Tg_mult(ii-count)=[];
...
%% Final Plot
irrad=[0,1*10^11, 3.5*10^10, 1*10^10, 3.5*10^11, 1*10^12];
Tg_rad=[116.3183, 82.0996, 100.05, 97.9077, 87.27, 86.1];
y_err=[1.187, 2.0782, 1.3301, 2.34, 1.99, 4.96];

figure(12)
%semilogx(irrad,Tg_rad, 'o')
...
%%
writematrix(M, '1E10_heatflow_col.txt');
writematrix(T, '1E10_time_col.txt');

%%
writematrix(Tg_mult, 'Tg_1E10.dat')
writematrix(Tg_mult_std, 'Tg_std_1E10.dat')
